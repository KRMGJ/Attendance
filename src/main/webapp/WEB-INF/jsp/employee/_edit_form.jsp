...
<div class="mx-auto max-w-2xl">
	<div class="bg-white border rounded-xl p-6 shadow-sm">
		...
		<form id="employeeEditForm" method="post" action="<c:url value='/employee/edit.do'/>" class="grid grid-cols-1 md:grid-cols-2 gap-4">
			...
			<div class="col-span-1">
				...
				<input name="employeeNumber" id="employeeNumberEdit" value="${employee.employeeNumber}" class="w-full rounded border px-3 py-2" />
				<p class="text-xs text-gray-500 mt-1">영문 대문자와 숫자만 사용 가능합니다. (예: EMP001)</p>
			</div>

			<div class="col-span-1">
				<label class="block text-sm mb-1">비밀번호 변경</label>
				<input type="password" name="password" placeholder="변경 시에만 입력" class="w-full rounded border px-3 py-2" />
				<p class="text-xs text-gray-500 mt-1">비밀번호를 변경하지 않으려면 비워 두세요.</p>
			</div>
			...
			<div class="col-span-1">
				...
				<input name="phone" id="phoneEdit" value="${employee.phone}" placeholder="예: 010-1234-5678" class="w-full rounded border px-3 py-2" />
				<p class="text-xs text-gray-500 mt-1">숫자, -, +, (), 공백만 사용 가능합니다.</p>
			</div>

			<div class="col-span-1">
				<label class="block text-sm mb-1">비상 연락처</label>
				<input name="emergencyContactPhone" id="emergencyPhoneEdit" value="${employee.emergencyContactPhone}"
			...
			<div class="col-span-2">
				...
				<input name="address" value="${employee.address}" class="w-full rounded border px-3 py-2" />
			</div>
			...
			<div class="col-span-2 flex justify-end gap-2 mt-2">
				...
				<button type="submit" class="px-4 py-2 rounded-md bg-gray-900 text-white">저장</button>
			</div>
		</form>
	</div>
</div>
